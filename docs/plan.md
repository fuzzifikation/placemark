...
| 3   | **Stats → Filters** (clickable format/camera rows + chips) | 7     | ✅ Complete    | Stats panel is now a floating glass panel (non-blocking). Format/camera rows are clickable filters. Active chips shown in strip below the header. Map controls shift to avoid overlap. |
...
- **v0.9.5 — UI polish & operations reliability (Apr 2026):** Compact header mode: `useWindowWidth` hook drives icon-only layout below 1100px, making the app usable at 1024px. All floating-header buttons now have `title` tooltips. Placemarks panel import/export icons swapped to `FolderOutput`/`FolderInput` for clarity. Operations service modularized into focused modules; fixed lock-release guarantee and trash-failure propagation by `photoId`.
